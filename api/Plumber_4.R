...
# -----------------------------
BASE_DIR <- Sys.getenv("MODEL_DIR", file.path(getwd(), "models"))

paths <- list(
  # 14-day Death
  clinical_base = file.path(BASE_DIR, "Final_Model/Death/Clinical/Original/Clinical_Original_Death_Lasso_1se_bundle.rds"),
  clinical_alt  = file.path(BASE_DIR, "Final_Model/Death/Clinical/Alternative/Clinical_A_Death_lasso_1se_bundle.rds"),
  ct_base       = file.path(BASE_DIR, "Final_Model/Death/CT/Original/CT_Original_Death_EN_a0.6_1se_bundle.rds"),
  ct_alt        = file.path(BASE_DIR, "Final_Model/Death/CT/Alternative/CT_A_Death_EN_a0.6_1se_bundle.rds"),
  full_base     = file.path(BASE_DIR, "Final_Model/Death/Full/Original/Full_Original_Death_lasso_1se_bundle.rds"),
  full_alt      = file.path(BASE_DIR, "Final_Model/Death/Full/Alternative/Full_A_Death_Lasso_1se_bundle.rds"),
  
  # 3-Month
  `3m_clinical_base` = file.path(BASE_DIR, "Final_Model/3Month/Clinical/Original/Clinical_Original_3Month_Lasso_0.03_bundle.rds"),
  `3m_clinical_alt`  = file.path(BASE_DIR, "Final_Model/3Month/Clinical/Alternative/Clinical_A_3Month_Lasso_0.03_bundle.rds"),
  `3m_ct_base`       = file.path(BASE_DIR, "Final_Model/3Month/CT/Original/CT_Original_3Month_Lasso_0.025_bundle.rds"),
  `3m_ct_alt`        = file.path(BASE_DIR, "Final_Model/3Month/CT/Alternative/CT_A_3Month_Lasso_0.025_bundle.rds"),
  `3m_full_base`     = file.path(BASE_DIR, "Final_Model/3Month/Full/Original/Full_Original_3Month_Lasso_0.02_bundle.rds"),
  `3m_full_alt`      = file.path(BASE_DIR, "Final_Model/3Month/Full/Alternative/Full_A_3Month_Lasso_0.02_bundle.rds"),
  
  # 6-Month
  `6m_clinical_base` = file.path(BASE_DIR, "Final_Model/6Month/Clinical/Original/Clinical_Original_6Month_Lasso_0.025_bundle.rds"),
  `6m_clinical_alt`  = file.path(BASE_DIR, "Final_Model/6Month/Clinical/Alternative/Clinical_A_6Month_Lasso_0.025_bundle.rds"),
  `6m_ct_base`       = file.path(BASE_DIR, "Final_Model/6Month/CT/Original/CT_Original_6Month_EN_a0.3_l0.05_bundle.rds"),
  `6m_ct_alt`        = file.path(BASE_DIR, "Final_Model/6Month/CT/Alternative/CT_A_6Month_EN_a0.3_l0.05_bundle.rds"),
  `6m_full_base`     = file.path(BASE_DIR, "Final_Model/6Month/Full/Original/Full_Original_6Month_EN_a0.5_l0.04_bundle.rds"),
  `6m_full_alt`      = file.path(BASE_DIR, "Final_Model/6Month/Full/Alternative/Full_A_6Month_EN_a0.5_l0.04_bundle.rds"),
  
  # 12-Month
  `12m_clinical_base` = file.path(BASE_DIR, "Final_Model/12Month/Clinical/Original/Clinical_Original_12Month_lasso0.025_bundle.rds"),
  `12m_clinical_alt`  = file.path(BASE_DIR, "Final_Model/12Month/Clinical/Alternative/Clinical_A_12Month_lasso0.025_bundle.rds"),
  `12m_ct_base`       = file.path(BASE_DIR, "Final_Model/12Month/CT/Original/CT_Original_12Month_EN_a0.4_l0.04_bundle.rds"),
  `12m_ct_alt`        = file.path(BASE_DIR, "Final_Model/12Month/CT/Alternative/CT_A_12Month_EN_a0.4_l0.04_bundle.rds"),
  `12m_full_base`     = file.path(BASE_DIR, "Final_Model/12Month/Full/Original/Full_Original_12Month_ridge_0.08_bundle.rds"),
  `12m_full_alt`      = file.path(BASE_DIR, "Final_Model/12Month/Full/Alternative/Full_A_12Month_ridge_0.08_bundle.rds")
)
...
predict_by_coef <- function(bd, newdata, strict = TRUE, return_debug = TRUE){
  ...
  Xp <- Xp[, want, drop = FALSE]   # ← 이 줄이 핵심 수정
  
  btab <- setNames(bd$coef_dense, bd$coef_names)
  b0   <- unname(btab["(Intercept)"])
  beta <- btab[setdiff(names(btab), "(Intercept)")]
  eta  <- as.numeric(b0 + Xp %*% as.numeric(beta[colnames(Xp)]))
  p    <- stats::plogis(eta)
  
  if (!return_debug) return(p)
  
  ns_in_want <- grep("^ns\\(", want, value = TRUE)
  lab_coef_sum <- sum(abs(beta[ns_in_want]), na.rm = TRUE)
  note <- if (lab_coef_sum == 0) "이 번들에서 스플라인(Lab) 계수 합이 0입니다." else NULL
  ...
  list(
    ...
    debug = list(
      miss_before = miss_before,
      extra_before = extra_before,
      ns_in_want = ns_in_want,
      lab_coef_L1sum = unname(lab_coef_sum),
      strict = strict,
      note = note
    )
  )
